...
class TestSidebarControls:
    ...
    def test_sidebar_has_area_selection_radio(self, app):
        radios = app.radio
        area_radio = next((r for r in radios if r.label == "Tipo de seleção"), None)
        assert area_radio is not None
        assert set(area_radio.options) == {"Desenhar no mapa", "Área pré-definida"}

    def test_sidebar_predefined_area_selectbox_when_selected(self, app):
        app.radio[0].set_value("Área pré-definida").run()
        selectboxes = app.selectbox
        area_select = next((sb for sb in selectboxes if sb.label == "Área pré-definida"), None)
        assert area_select is not None
        assert "Talhão A" in area_select.options
...
class TestInitialState:
    def test_initial_message_displayed(self, app):
        ...
        assert any("Selecione uma área e clique em Analisar" in msg for msg in info_messages)


class TestPredefinedAreas:
    def test_predefined_areas_loads_options(self, app):
        app.radio[0].set_value("Área pré-definida").run()
        selectboxes = app.selectbox
        area_select = next((sb for sb in selectboxes if sb.label == "Área pré-definida"), None)
        assert area_select is not None
        assert len(area_select.options) >= 3
        assert "Talhão A" in area_select.options
        assert "Talhão B" in area_select.options
        assert "Talhão C" in area_select.options